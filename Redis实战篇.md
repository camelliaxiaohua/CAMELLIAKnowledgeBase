...
* **优惠卷秒杀**：Redis的计数器功能， 结合Lua完成高性能的redis操作，同时学会Redis分布式锁的原理，包括Redis的三种消息队列
...
### 1.1. 前后端分离 + 多级缓存 + 分布式集群架构
![1653059409865](https://camelliaxiaohua-1313958787.cos.ap-shanghai.myqcloud.com/markdown/1653059409865-20250409215259974-20250409215403607.png)

想象一下，你用手机打开一个购物 App，点击首页的时候，其实你就是发出了一个请求，这个请求首先会来到我们的大门口——**Nginx 服务器**。

Nginx 就像一个高效、多才多艺的“门卫”：

- 如果你只是来看图片、打开静态网页（比如首页的横幅图、产品列表），Nginx 自己就能直接把内容拿给你看，根本不用麻烦后面的系统；
- 如果你要查购物车、登录账号这些需要处理逻辑的请求，Nginx 会把这些“复杂的请求”分发到后面的 Tomcat 系统处理，就像把顾客引导到不同的柜台排队。

更厉害的是，Nginx 还能写点小程序（比如 Lua 脚本），它甚至可以直接从 Redis 这个“高速缓存库”里取数据，根本不用叫醒后台的 Tomcat，速度自然更快。

为什么需要 Nginx 分流呢？我们来打个比方：

假设后台的 Tomcat 服务器就像是服务员，每一台服务器的配置（比如 4 核 CPU + 8G 内存）就只能同时接待 1000 个客户。如果成千上万人一拥而上，那服务员一下就“崩了”。但有了 Nginx 这个前台，它可以把人群合理分流，让不同的服务员（Tomcat）各自负责一部分人，压力就平均了，系统也就稳定了。

而且，前端页面（JS、CSS、图片等）我们也可以部署在 Nginx 上，这样客户打开网页时，大部分资源都是直接从 Nginx 拿的，不需要走后台。**这就叫“动静分离”**——动态的交给 Tomcat，静态的交给 Nginx，大大减轻了后台压力。

接下来我们再看数据怎么来的。

当 Tomcat 接到请求，比如用户想看“我的订单”，它需要从数据库里查，这时候就要访问 MySQL。

但 MySQL 是个“数据金库”，读写成本很高，就算是配置豪华的企业级数据库（比如 32 核 CPU、64G 内存），也只能承受 4000～7000 个并发请求。如果大家一拥而上，数据库立刻就“爆仓”：CPU、硬盘被打满，系统崩溃。

所以我们不能让所有请求都直接打到数据库去，而是：

1. **用 Redis 做缓存**，像个“临时储物柜”，热点数据优先从 Redis 拿，不用每次都翻箱倒柜去 MySQL；
2. **用 MySQL 集群**，不是一台数据库扛，而是好几台一起上，分工协作，分担压力。

通过这种组合拳：**Nginx 负责“接客分流 + 静态资源 + 快速路径访问”，Tomcat 负责业务逻辑，Redis 负责热点缓存，MySQL 做最后的保障支撑**，我们就能搭建一个即高效、又能抗大流量的系统架构。
...
用户在提交**手机号/邮箱**后，会校验**手机号/邮箱**是否为空或者合法。如果为空或者不合法，则要求用户重新输入**手机号/邮箱**。

如果**手机号/邮箱**合法，后台此时生成对应的验证码，同时将验证码进行保存，然后再通过邮箱或者短信的方式将验证码发送给用户

- **短信验证码登录/注册**

用户将**验证码**、**手机号/邮箱**和**密码**进行输入，后台从**session**中拿到当前验证码。首先，对入参进行判空校验，接着对**手机号/邮箱**的合法性进行校验。然后，和用户输入的验证码进行校验，如果不一致，则无法通过校验。如果一致，则后台根据**手机号/邮箱**查询用户。如果用户不存在，则为用户创建账号信息，保存到数据库。如果用户存在，还需要对用户的密码进行校验。无论是否存在，都会将用户信息保存到session中，方便后续获得当前登录信息。

- **校验登录状态**

用户在请求时候，会从**cookie**中携带者**JsessionId**到后台，后台通过JsessionId从session中拿到用户信息，如果没有session信息，则进行拦截，如果有session信息，则将用户信息保存到**ThreadLocal**中，并且放行。
...
```java
@Override
public ApiResponse<String> loginOrRegister(String phoneNumber, String password, String captcha, H
    // 3. 手机号合法校验
    if (RegexUtils.isPhoneInvalid(phoneNumber)){
...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        // 1. 获取 session 和 token
        HttpSession session = request.getSession();
        // 2. 获取用户
        UserInfo user = (UserInfo) session.getAttribute("user");
        // 3. 若 session 用户不存在，拦截
        if (user == null) {
        ...
        // 4. 若 session 用户存在，放行
        response.addDateHeader("LoginDate", System.currentTimeMillis());
   ...
   @Override
	 public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
    		// 请求完成后，清理 ThreadLocal 数据，避免数据泄漏。
    		UserThreadLocal.removeUserInfo();
		}
}
```

> 用户信息**存储在 `Session` 中**。在请求进来时，通过拦截器拦截请求。**从 `Session` 中获取用户信息**，并将其存放到 `ThreadLocal` 中。以便后续的业务代码可以通过 `ThreadLocal` 获取当前线程相关的用户信息。
...
当用户通过浏览器或 App 发起请求时，请求会被发送到 Tomcat 所监听的端口（比如 8080）。Tomcat 启动后，会由一个称为 **Acceptor（接受者）线程** 专门负责监听这个端口。只要它检测到有新的请求接入，就会通过操作系统底层的 socket 接口，建立一个与客户端的连接。这个连接是“成对”的，一端在客户端，一端在 Tomcat 服务端。

连接建立后，Tomcat 并不会让监听线程来处理请求，而是会将这个连接交由一个 **线程池（Executor）** 来处理。线程池会从中挑选一个空闲的工作线程，分发任务给它。这一线程负责读取请求内容、路由到对应的 Web 应用，然后依次执行我们写在其中的 Controller、Service、DAO 等代码逻辑。如果有数据库操作，还会访问数据库完成数据的查询或更新。

当业务逻辑处理完成后，工作线程会将响应内容通过 socket 写回给客户端。这个过程标志着一个完整请求-响应周期的结束。此时线程并不会销毁，而是会被归还给线程池，等待下一次分配任务，从而大大节省了线程创建和销毁的开销。

由于 Tomcat 是**为每一个请求分配一个线程**来处理，因此我们可以借助 Java 提供的 **`ThreadLocal`** 机制来实现线程间的数据隔离。`ThreadLocal` 本质上是为当前线程开辟一块“私有空间”，只能被当前线程访问。这样，在业务处理过程中，像“当前登录用户信息”、“数据库连接”、“事务上下文”等信息，就可以安全地存储在 `ThreadLocal` 中，不会被其他线程看到或篡改，从而确保线程安全与数据隔离。

> [!CAUTION]
>
> ThreadLocal 中保存的信息，在请求结束后必须手动清理，否则可能造成数据“复用”或“内存泄漏”。如果新的请求刚好复用了旧线程，且没有清理 ThreadLocal，就可能拿到之前的旧数据。

虽然 ThreadLocal 可以让每个线程拥有独立变量，但线程是复用的，不清理就等于让新来的请求踩进“别人家的坑”。就像一个酒店房间，住客退房了却不打扫，下一位住客进来会发现“床上还有前一个人的行李”，这就非常危险。所以一定要在使用完 ThreadLocal 后，记得打扫战场（调用 remove）！



### 1.4. session共享问题

当我们把应用部署在多台服务器上组成集群时，就可能遇到一个典型问题——**Session 共享问题**。这是因为用户登录后的会话信息通常保存在服务器的内存中，也就是 **`HttpSession` 默认存在于 Tomcat 的 JVM 中**。但是在集群环境中，用户的请求可能被 Nginx 等负载均衡器随机分发到任意一台服务器，这就可能出现用户第一次访问的是 **Tomcat A**，Session 被保存在 A 上，而第二次请求却被分发到了 **Tomcat B**，结果 B 上并没有这个用户的登录信息，系统就会误以为用户未登录，导致“登录失效”等问题。

> [!NOTE]
>
> **`HttpSession` 默认是保存在 Tomcat 的内存（JVM 堆内存）里**，这也是为什么在高并发或者用户量特别大的系统中，**直接使用 HttpSession 会有内存风险**。
>
>  **JVM 最大可用内存 / 每个 Session 占用的内存 ≈ 可支持的最大 Session 数**，保守估计**一台 2GB JVM 的服务器，最多承载约 5 万 ~ 10 万个活跃 Session**。

造成这个问题的根本原因在于，默认的 `HttpSession` 是**本地内存存储**，不同服务器之间并不会自动同步 Session 数据。也就是说，Session 是“绑死”在单个服务器节点上的，而不是“跟着用户走”的。这就导致在请求打散的情况下，用户的登录状态无法在集群中保持一致。

为了解决这个问题，常见的方法有三种。

#### 1.4.1. Nginx 配置**“会话粘性”**

第一种是配置 Nginx 的**会话粘性（Sticky Session）**策略。也就是让 Nginx 根据客户端 IP 地址，**将来自同一个 IP 的请求始终路由到同一台服务器上**。这样虽然能暂时解决 Session 不一致的问题，但如果该服务器宕机，用户的会话就会丢失，而且扩展性差。

```nginx
upstream backend {
    ip_hash; # 按 IP 分发，保证相同 IP 请求命中同一台服务器
...
第二种方法也是最推荐的方式——使用 Redis 等中间件进行**集中式 Session 管理**。比如使用 Spring Session 框架将 Session 存入 Redis，无论用户的请求落在哪台服务器上，大家都从同一个 Redis 中取 Session，就不会出现 Session 丢失问题。这种方式不仅解决了共享问题，还具备良好的可扩展性和高可用性。

#### 1.4.3. Token + Stateless 模式

第三种方法是采用更现代的无状态方案，也就是通过 **Token（如 JWT）机制**来管理用户状态。用户登录成功后，服务器生成一个 Token 并返回给前端。后续每次请求都携带这个 Token，服务器根据它来识别用户，无需在服务器端维护 Session 状态。这种方式具有更高的性能和更强的扩展能力，但也需要注意 Token 安全和管理，比如过期时间、签名校验等。

> [!TIP]
>
> **在分布式系统中，不建议依赖默认的 HttpSession，而应通过 Redis 实现 Session 共享，或干脆采用无状态的 Token 模式**。



### 1.5. Redis解决session信息共享

Redis 是一种高性能、支持持久化的内存数据库，可以作为 Session 的统一存储中心，所有节点都从 Redis 读取/写入 Session，就实现了共享。这不仅解决了多节点间的 Session 不一致问题，还可以将原本存储在每台 Tomcat 内存中的 Session 数据集中到 Redis 中，从而**缓解 JVM 的内存压力**，避免因 Session 过多而导致内存溢出（**OOM**）的风险，提高系统的稳定性和扩展能力。

#### 1.5.1. Redis数据类型选型

**String 类型**是 Redis 最基础的数据结构，它可以存储任何类型的值（字符串、数字、JSON 等），本质上是一个 `key -> value` 的键值对。查询是直接通过 key 找到 value，O(1) 操作。

```json
set user:1 "{\"name\":\"张三\",\"age\":25,\"address\":\"北京\"}"
```

**Hash 类型**可以理解为 Redis 中的小型哈希表（Map），是一个 `key -> field -> value` 的三层结构。可以在一个 key 下维护多个字段。 对小型 Hash（field < 1000，field+value 都不大于 64 字节）会自动使用**ziplist（紧凑列表）或 listpack 编码**进行压缩，进一步降低内存占用。查询时是通过 key 找 Hash，再通过 field 找 value，也接近 O(1)，**速度几乎和 String 持平**。

```json
hset user:1 name 张三
hset user:1 age 25
hset user:1 address 北京
```
...
系统获取用户输入的邮箱后，首先检查是否为空，并校验邮箱格式是否合法。通过校验后，查询 Redis 中该邮箱请求验证码的次数，判断是否超过频率限制，并检查是否已有未失效的验证码。

如果未超限且没有有效验证码，系统生成新的验证码，保存到 Redis 中，并异步发送到对应邮箱。若发送失败，清除 Redis 中的验证码记录；发送成功则流程结束。

- **短信验证码登录/注册**

系统接收邮箱、密码和短信验证码后，首先校验这些字段是否为空，然后验证邮箱格式是否正确。之后从 Redis 中获取验证码，若验证码不存在或错误，则流程结束。

接着，系统根据邮箱查询用户信息，如果用户不存在，则新建用户。如果用户已存在，检查密码错误次数，并进行限制。如果错误次数超过限制，锁定用户一段时间。若密码正确，重置错误计数器。

最终，系统使用 JWT 存储用户信息，并返回成功。

- **校验登录状态**

系统创建请求拦截器后，从请求头中获取 `authorization` 信息。若信息存在，构造对应的 Redis key，并尝试从 Redis 中获取用户信息。

如果用户信息存在，则将其存入 `ThreadLocal`，并刷新 Redis 中的过期时间，随后放行请求。
...
public Boolean sendCaptcha(String email, HttpSession session) {
    ...
    try {
        // 7. 存储验证码（原子操作）
        stringRedisTemplate.opsForValue().set(captchaKey, captcha, RedisConstants.CAPTCHA_EMAIL_
        // 8. 异步发送邮件
...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 2. 若 authorization 为空，拦截请求
        if (StrUtil.isBlank(authorization)) {
            response.setStatus(401);
            response.getWriter().write("{\"code\":401,\"msg\":\"未登录\"}");
            return false;
        }
        // 3. 若 authorization 不为空，从 redis 中获取用户信息
        String userStr = stringRedisTemplate.opsForValue().get(RedisConstants.USER_INFO_PREFIX + authorization);
        ...
        UserThreadLocal.setUserInfo(user);
        // 6. 刷新 redis 中用户信息的过期时间
        stringRedisTemplate.expire(RedisConstants.USER_INFO_PREFIX + authorization, RedisConstants.USER_INFO_TTL, TimeUnit.MINUTES);
        // 7. 放行请求
        return true;
    }
}
